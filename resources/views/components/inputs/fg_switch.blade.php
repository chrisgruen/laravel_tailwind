        ...
        @if(isset($realArray)&&$realArray===true)
            @foreach($options as $key=>$val)
                <div class="mb-4 toggle-check-button">
                    <input id="{{isset($key_val)?$key:str_replace(' ', '_', $val)}}" type="checkbox" name="{{$slot}}[]" data-toggle="toggle" value="{{isset($key_val)?$key:$val}}" @if(isset($value_array)) @if(in_array(isset($key_val)?$key:$val, $value_array)) checked @endif @endif data-size="sm" data-width="75" data-on="{{isset($txt_on)?$txt_on:trans('messages.yes')}}" data-off="{{isset($txt_on)?$txt_on:trans('messages.no')}}">
                    <label for="{{isset($key_val)?$key:str_replace(' ', '_', $val)}}" class="text-gray-700 pl-6 mb-0">&nbsp; {{$val}}</label>
                </div>
            @endforeach
